/**
 * funk: das junge Content-Netzwerk von ARD und ZDF. Ported from the v1 addon.
 *
 * Served through ARD's page gateway (the same API the ARD Mediathek uses)
 * with org "funk": the home page lists the categories, a category pages its
...
const fetchJson = async (url) => {
  ...
  if (!res.ok) throw new Error(`funk ${res.status} on ${url}`);
  return res.json();
};
...
async function loadItem(id) {
  ...
  // funk content is often hosted under hr, ndr, daserste, zdf, …
...
const valueOf = (segment) => {
  ...
  return raw.startsWith("funk:") ? raw.slice(5) : raw;
};
...
async function manifest() {
  ...
  return {
    ...
    version: "2.0.0",
    description:
      "funk: das junge Content-Netzwerk von ARD und ZDF: Reportagen, Comedy, Musik, Doku und mehr. Kostenlos, ohne API-Key. Streams sind i.d.R. auf Deutschland geo-beschränkt.",
    icon: "https://www.funk.net/img/favicons/favicon-228x228.png",
